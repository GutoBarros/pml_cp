...
Although the instructions for this project grants the use of any feature available on the training dataset, we decide to stick with the principles of the original paper and kept only the measurement variables, as the original main goal was to use the measures of movement as predictors, not the metadata. So, we decided to drop the metadata features and go on with the measurement ones.
...
We try of Principal Component Analysis (PCA) to reduce dimensionality seems to be a good approach in this case. We kept 90% of the original variance, ending with 20 components. It is notewothy that centering and scaling the variables are carried out along with the PCA pre process.
...
A XGBoost model was trained on the data processed with PCA by using the Caret package. We used the method "xgbtree" that needs seven parameters:  
...
The grid search for those parameters comprised 243 models (see [The Caret Reference, Basic parameter Tuning](https://topepo.github.io/caret/model-training-and-tuning.html#basic-parameter-tuning)), each of then was cross validated in ten folds (k = 10). The final parameters were:
...
if(file_test("-f", "ml_xgboost1.RDS") == FALSE) 
  {
...
ml_xgboost1 <- readRDS("ml_xgboost1.RDS")
ml_xgboost1$finalModel
...
confusionMatrix(predict(ml_xgboost1, newdata = training[,-1]), reference = as.factor(training[,1]))
...
testing <- predict(preProc, testing) # preprocessing (PCA) testdata using preprocess from training data

confusionMatrix(predict(ml_xgboost1, newdata = testing[,-1]), reference = as.factor(testing[,1]))
